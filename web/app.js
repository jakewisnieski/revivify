...
// --- Design-advisory panel (M6.4) -----------------------------------------
// A separate, opt-in surface: the deterministic dial renders instantly; the
// slow (~30–60s), availability-gated design layer runs only when asked. It is
// never scored and never touches the dial (decision #31).
...
function renderDesign(panel) {
  ...
  if (panel.status === "unavailable") {
    // The reason is already a full phrase (e.g. "design analysis timed out"); just
    // capitalize it rather than prefixing another "Design analysis unavailable —".
    const reason = panel.reason || "design analysis unavailable";
    const msg = reason.charAt(0).toUpperCase() + reason.slice(1);
    body.innerHTML =
      `<div class="design-unavailable">${esc(msg)}.` +
      `<div class="muted">${esc(panel.note)}</div></div>`;
    return;
  }
  let html = "";
  for (const g of panel.groups) {
    html +=
      `<div class="design-group"><div class="design-group-head">` +
      `<span class="tier-chip ${TIER_CLASS[g.tier] || "t3"}">Tier ${g.tier} · ${esc(g.label)}</span>` +
      ` <span class="muted">${esc(g.caption)}</span></div><ul class="design-list">`;
    for (const it of g.items) {
      const tag = it.opinion
        ? `<span class="chip info">model opinion</span>`
        : `<span class="design-cite">${esc(it.citation)}</span>${learnMore(it)}`;
      html +=
        `<li><div class="design-item-title">${esc(it.title)} ${tag}</div>` +
        `<div class="design-item-detail">${esc(it.detail)}</div></li>`;
    }
    html += `</ul></div>`;
  }
  if (!panel.groups.length) {
    html += `<div class="muted">No specific suggestions this time — but that’s not proof the design is good (see below).</div>`;
  }
  html += `<div class="design-disclaimer muted">${esc(panel.disclaimer)}</div>`;
  if (panel.model) html += `<div class="design-model muted">Generated by ${esc(panel.model)} · advisory</div>`;
  body.innerHTML = html;
}

async function runDesign() {
  const btn = $("design-run");
  const body = $("design-body");
  btn.disabled = true;
  body.innerHTML = `<div class="design-loading"><span class="spinner" aria-hidden="true"></span> Analyzing the design — this runs a local vision model and can take ~30–60s…</div>`;
  try {
    const path = encodeURIComponent($("path").value.trim());
    const res = await fetch(`/api/design?path=${path}`);
    renderDesign(await res.json());
  } catch (e) {
    body.innerHTML =
      `<div class="design-unavailable">Design analysis unavailable — ${esc(e.message || "request failed")}.` +
      `<div class="muted">The deterministic gate and trust score are unaffected.</div></div>`;
  } finally {
    btn.disabled = false;
    btn.textContent = "Re-run design suggestions";
  }
}

// Reset the design card to its opt-in state for a fresh check — stale
// suggestions never linger next to a newly-scored page.
function resetDesign() {
  $("design-body").innerHTML = "";
  const btn = $("design-run");
  btn.disabled = false;
  btn.textContent = "Get design suggestions";
  $("design-card").classList.remove("hidden");
}
...
function render(out) {
  ...
  resetDesign(); // advisory ceiling: opt-in, reset per check, never part of the score
...
$("design-run").addEventListener("click", runDesign);
